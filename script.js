const proxy="https://proxy.ikunbeautiful.workers.dev/?url=",tabs=[],history=JSON.parse(localStorage.getItem("browserHistory")||"[]"),bookmarks=JSON.parse(localStorage.getItem("browserBookmarks")||"[]");let activeTab=null;

// ---------- Dark Mode ----------
const themeBtn=document.getElementById("theme");
const setTheme=dark=>{document.body.classList.toggle("dark",dark);themeBtn.innerHTML=`<span class="material-icons">${dark?"light_mode":"dark_mode"}</span>`;localStorage.setItem("darkMode",dark?"1":"0");};
...
// ---------- Tabs ----------
const favicon=url=>{try{return"https://www.google.com/s2/favicons?domain="+new URL(url).hostname;}catch{return"";}};
...
function renderTabs(){
  ...
  tabs.forEach(t=>{
    ...
    tabEl.onmouseenter=e=>{
      preview.innerHTML=t.iframe.src.startsWith(window.location.origin)?(()=>{const c=t.iframe.cloneNode(true);c.style.pointerEvents='none';return preview.appendChild(c);})() : `<div style="padding:10px;">${t.title||t.url}</div>`;
      preview.style.display='block';preview.style.left=e.pageX+'px';preview.style.top=(e.pageY+20)+'px';
    };
...
function handleDrop(e){e.stopPropagation();if(dragSrcEl!==e.currentTarget){const f=tabs.findIndex(t=>t.id==dragSrcEl.dataset.id),to=tabs.findIndex(t=>t.id==e.currentTarget.dataset.id);tabs.splice(to,0,tabs.splice(f,1)[0]);renderTabs();attachDragEvents();}}
...
function newTab(url="https://proxy.jimmyqrg.com/default/"){
  const id=Date.now(),iframe=document.createElement("iframe");
  iframe.src=proxy+encodeURIComponent(url);iframe.style.display="none";iframe.dataset.id=id;
  ...
  const tab={id,url,title:url,iframe};tabs.push(tab);switchTab(id);
}
