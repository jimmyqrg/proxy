...
function handleDrop(e){
  ...
  if(dragSrcEl !== e.currentTarget){
    const fromIndex = tabs.findIndex(t=>t.id==dragSrcEl.dataset.id);
    const toIndex = tabs.findIndex(t=>t.id==e.currentTarget.dataset.id);
    tabs.splice(toIndex,0,tabs.splice(fromIndex,1)[0]);
    renderTabs(); attachDragEvents();
  }
}
function attachDragEvents(){
  document.querySelectorAll('.tab').forEach(tab=>{
    tab.draggable=true;
    tab.dataset.id = tabs.find(t=>t.title===tab.querySelector('span').innerText).id;
...
function newTab(url="default/"){  // default tab page
  const id=Date.now();
...
function switchTab(id){
  ...
  if(activeTab){ activeTab.iframe.style.display="block"; document.getElementById("url").value=activeTab.url; renderTabs(); attachDragEvents(); }
}
